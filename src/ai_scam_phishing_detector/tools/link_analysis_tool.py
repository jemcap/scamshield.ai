...
class LinkAnalysisInput(BaseModel):
    ...
    content: str = Field(..., description="The message content to extract and analyze links from")


class LinkAnalysisOutput(BaseModel):
    """Output schema for LinkAnalysisTool."""
    extracted_urls: List[str] = Field(..., description="List of URLs found in the content")
    analysis_results: List[Dict[str, Any]] = Field(..., description="Detailed analysis for each URL")
    overall_risk: str = Field(..., description="Overall risk assessment: LOW, MEDIUM, HIGH, CRITICAL")
...
class LinkAnalysisTool(BaseTool):
    ...
    def _check_url_shorteners(self, url: str) -> Dict[str, Any]:
        ...
        shortener_services = [
            ...
            'ow.ly', 'buff.ly', 'tiny.cc', 'is.gd', 'v.gd', 'rb.gy',
            'cutt.ly', 'short.io', 'rebrand.ly', 'bl.ink'
        ]
    ...
    def _analyze_domain_safety(self, url: str) -> Dict[str, Any]:
        """Analyze domain for safety indicators."""
        parsed = urlparse(url)
        domain = parsed.netloc.lower()
        ...
        if re.search(r'\d+\.\d+\.\d+\.\d+', domain):  # IP address
            safety_analysis['suspicious_indicators'].append('Uses IP address instead of domain')
        ...
        if re.search(r'[0-9]{4,}', domain):  # Long numbers in domain
            safety_analysis['suspicious_indicators'].append('Contains long numeric sequences')
            safety_analysis['risk_score'] += 15
        
        # Check for suspicious TLDs
        suspicious_tlds = ['.tk', '.ml', '.ga', '.cf', '.click', '.download', '.top']
        if any(tld in domain for tld in suspicious_tlds):
            safety_analysis['suspicious_indicators'].append('Uses suspicious top-level domain')
            safety_analysis['risk_score'] += 25
        
        # Check for typosquatting of popular domains
        popular_domains = ['amazon', 'paypal', 'google', 'microsoft', 'apple', 'facebook', 'bank']
        for pop_domain in popular_domains:
            if pop_domain in domain and not domain.startswith(f'{pop_domain}.'):
                safety_analysis['suspicious_indicators'].append(f'Possible typosquatting of {pop_domain}')
                ...
                if domain_info.creation_date:
                    ...
                    if creation_date:
                        ...
                        if domain_age < 30:  # Very new domain
                            safety_analysis['suspicious_indicators'].append('Domain registered very recently')
                            safety_analysis['risk_score'] += 40
                        elif domain_age < 90:  # New domain
                            safety_analysis['suspicious_indicators'].append('Domain registered recently')
                            safety_analysis['risk_score'] += 20
        except:
            pass
    ...
    def _check_malicious_patterns(self, url: str) -> Dict[str, Any]:
        ...
        malicious_patterns = {
            ...
            'suspicious_paths': [
                '/phish/', '/scam/', '/fake/', '/clone/', '/mirror/',
                '/proxy/', '/redirect/', '/forward/'
            ],
            'suspicious_params': [
                'redirect', 'url', 'link', 'goto', 'target', 'continue',
                'return', 'next', 'forward'
            ]
        }
        ...
        if '%' in url and len(re.findall(r'%[0-9a-fA-F]{2}', url)) > 3:
            patterns_found.append('URL contains excessive encoding (possible obfuscation)')
            risk_score += 20
                ...
                try:
                    ...
                    if result and 'organic' in result:
                        for item in result['organic']:
                            ...
                            malicious_keywords = ['malicious', 'phishing', 'scam', 'malware', 'blacklist', 'dangerous']
                            if any(keyword in title + snippet for keyword in malicious_keywords):
    ...
    def _run(self, content: str) -> str:
        """Analyze all URLs found in the content."""
        try:
            ...
            for url in urls:
                ...
                url_analysis = {
                    ...
                    'domain_safety': self._analyze_domain_safety(url),
            ...
            # Generate summary
            summary = self._generate_summary(urls, analysis_results, highest_risk, malicious_urls)
...
"""
                ...
                for report in analysis['reputation']['malicious_reports'][:3]:  # Show top 3
    ...
    def _generate_summary(self, urls: List[str], analyses: List[Dict], overall_risk: str, malicious_urls: List[str]) -> str:
        """Generate a human-readable summary."""
        if not malicious_urls:
            return f"Analyzed {len(urls)} URL(s). No malicious links detected. Overall risk: {overall_risk}."
        
        summary = f"⚠️  WARNING: Found {len(malicious_urls)} potentially malicious URL(s) out of {len(urls)} total.\n\n"
